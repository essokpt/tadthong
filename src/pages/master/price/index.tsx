...
import { Price } from './components/schema' 
import { getItemPrice, searchPrice } from '@/services/itemApi'
...
export default function PriceMaster() {  
  ...
  const getData = () => {
    ...
    getItemPrice().then((data) => setData(data))
    setRefresh(false)
  }
  ...
  const queryData = (str:any) => {
    ...
    if(str == ''){
      ...
      searchPrice(str).then((data) => setData(data))
    }    
  }
  ...
  return (
    <Layout>
      ...
      <PageHeader
          label='Price Master'
          icon={<IconBusinessplan size={45} className='mt-2 ' />}
        />
